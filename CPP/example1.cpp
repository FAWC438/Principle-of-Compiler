...
char state[100];
int len_state;
//char move_set[100][100];
char start;
char final[100];
int len_final;
bool is_final_char[150];
...
void createNFA()
{
    ...
    while (N--)
    {
        ...
        if (strlen(str) > 1)
        {
            move_set[ch][str[0]].set[move_set[ch][str[0]].len++] = str[1];
        }
        else
        {
            //if(str[0]=='@')        //用@表示伊布熊
            //is_final_char[ch]=true;
            //else
            move_set[ch][str[0]].set[move_set[ch][str[0]].len++] = 'Y'; //终态
        }
    }
}
void showNFA()
{
    for (int i = 0; i < 100; ++i)
    {
        for (int j = 0; j < 100; ++j)
        {
            for (int k = 0; k < 100; ++k)
            {
                if (move_set[i][j].set[k] != '#')
                    cout << char(i) << " " << char(j) << " " << k << " " << move_set[i][j].set[k] << endl;
            }
        }
    }
}
...
void get_closure(NFA_set &temp) //得到一个完整的子集
{
    for (int i = 0; i < temp.len; ++i)
    {
        for (int j = 0; j < move_set[temp.set[i]]['@'].len; ++j)
        {
            if (!Is_in_set(move_set[temp.set[i]]['@'].set[j], temp))
            {
                temp.set[temp.len++] = move_set[temp.set[i]]['@'].set[j];
            }
        }
    }
}
...
void NFA_to_DFA()
{
    ...
    for (int i = 0; i < 150; ++i)
        is_final_char[i] = false;
    if (Is_contained_Y(work_set))
        is_final_char[num_new_set - 1] = true;
    while (!s.empty())
    {
        work_set = s.top();
        s.pop();
        for (int i = 0; i < len_final; ++i)
        {
            ...
            {
                for (int k = 0; k < move_set[work_set.set[j]][final[i]].len; ++k)
                {
                    if (move_set[work_set.set[j]][final[i]].set[k] != '#' && move_set[work_set.set[j]][final[i]].set[k] != 'Y' && !Is_in_set(move_set[work_set.set[j]][final[i]].set[k], worked_set))
                    {
                        worked_set.set[worked_set.len++] = move_set[work_set.set[j]][final[i]].set[k];
                    }
                    if (move_set[work_set.set[j]][final[i]].set[k] == 'Y' && !Is_in_set(move_set[work_set.set[j]][final[i]].set[k], worked_set))
                    {
...
bool DFA(char str[])
{
	...
	{
		now_state=move_set[now_state][str[i]];
		if(now_state==NULL)
			return false;
	}
	if(now_state=='0')
...
bool DFA(char str[])
{
    ...
    if (is_final_char[now_state] == true)
        return true;
    return false;
}
void scan()
